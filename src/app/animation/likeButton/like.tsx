...
const Like = ({
  ...
  toggle,
  ...props
}: prop & React.ComponentProps<'button'>) => {
  return (
    <button {...props} className="like" onClick={toggle}>
      <svg className="inner-svg" viewBox="0 0 24 24">
        <g>
          <path
            d={isLike ? FILLED_HEART : EMPTY_HEART}
            className={clsx(
              isLike
                ? 'filled-heart like-checked'
                : 'empty-heart color-transition'
            )}
          />
        </g>
      </svg>
